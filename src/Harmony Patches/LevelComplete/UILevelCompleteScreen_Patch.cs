    ...
    public static class UILevelCompleteScreen_Patch
    {
        ...
        public static void StartPostfix(UILevelCompleteScreen __instance)
        {
            try
            {
                ...
                // Apply font first
                ApplyFontToAllChildrenTMP(__instance, TMPFontReplacer.GetCachedFont(Plugin.GlobalFontPath), "[UILevelCompleteScreen]");
                ...
                if (canvasTrans != null)
                {
                    ...
                    if (logoTransform != null)
                    {
                        UITextureReplacer.ApplyTo(logoTransform.gameObject,
                            System.IO.Path.Combine(LanguageManager.CurrentSummary.Paths.TexturesDir, "UILogoText.png"), false);
                        Logging.Info("[UILevelCompleteScreen] Applied UILogoText.png");
                    }
                    else
                    {
        ...
        private static void WideScreenLevelCompletePatch(UILevelCompleteScreen __instance)
        {
            try
            {
                ...
                StretchHorizontal(levelNameBorderRt);
